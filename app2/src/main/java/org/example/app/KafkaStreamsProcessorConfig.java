...
public class KafkaStreamsProcessorConfig {
	...
	@Bean
	public KStream<String, SampleRecord> sampleRecordKStream(StreamsBuilder streamsBuilder,
			Serde<SampleRecord> avroSerde) {

		KStream<String, SampleRecord> stream = streamsBuilder.stream(inputTopic,
				Consumed.with(Serdes.String(), avroSerde));

		stream.mapValues(value -> new SampleRecord(value.getNumber() * -1, value.getText() + " - FINISH"))
