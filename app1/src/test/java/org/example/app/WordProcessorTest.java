...
/**
 * Integration test for the WordProcessor using an embedded Kafka broker.
 *
 * @SpringBootTest starts the Spring context.
 * @EmbeddedKafka starts an in-memory Kafka broker for the test.
 * - topics: pre-creates the specified topics.
 * - partitions: sets the number of partitions for the topics.
 * - brokerProperties: overrides Kafka broker properties. We point the
 * auto-created topics' offset reset to "earliest" to ensure consumers read from the beginning.
 * @TestInstance(TestInstance.Lifecycle.PER_CLASS) allows @BeforeAll and @AfterAll
 * to be non-static, which is convenient for managing instance variables.
 */
@SpringBootTest
@TestPropertySource(locations = "classpath:common.properties")
@EmbeddedKafka(
        topics = {"${app.kafka.topic.upstream}", "${app.kafka.topic.private}"},
        partitions = 1,
    ...
    void shouldTransformMessageToUppercase() throws InterruptedException {
        ...
        while (kafkaStreams.state() != KafkaStreams.State.RUNNING) {
            if (System.currentTimeMillis() > timeout) {
                throw new IllegalStateException("Kafka Streams did not reach the RUNNING state within the timeout. Current state: " + kafkaStreams.state());
            }
            Thread.sleep(100);
        }
        ...
        ConsumerRecord<String, SampleRecord> singleRecord = KafkaTestUtils.getSingleRecord(consumer, outputTopic, Duration.ofSeconds(10000L));
