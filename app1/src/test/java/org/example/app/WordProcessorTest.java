...
/**
 * Integration test for the WordProcessor using an embedded Kafka broker. @SpringBootTest
 * starts the Spring context. @EmbeddedKafka starts an in-memory Kafka broker for the
 * test. - topics: pre-creates the specified topics. - partitions: sets the number of
 * partitions for the topics. - brokerProperties: overrides Kafka broker properties. We
 * point the auto-created topics' offset reset to "earliest" to ensure consumers read from
 * the beginning. @TestInstance(TestInstance.Lifecycle.PER_CLASS) allows @BeforeAll
 * and @AfterAll to be non-static, which is convenient for managing instance variables.
 */
@SpringBootTest
@TestPropertySource(locations = "classpath:common.properties")
@EmbeddedKafka(topics = { "${app.kafka.topic.upstream}", "${app.kafka.topic.private}" }, partitions = 1,
...
public class WordProcessorTest {
	...
	// Autowire the StreamsBuilderFactoryBean to get access to the underlying KafkaStreams
	// instance.
	@Autowired
	...
	void shouldTransformMessageToUppercase() throws InterruptedException {
		...
		// This is a crucial step to prevent race conditions where the test sends a
		// message
		...
		// KafkaTestUtils.getSingleRecord polls the consumer for a message for a specified
		// duration.
		ConsumerRecord<String, SampleRecord> singleRecord = KafkaTestUtils.getSingleRecord(consumer, outputTopic,
		...
		assertThat(singleRecord.value().getText()).isEqualTo("HELLO WORLD");
	}
